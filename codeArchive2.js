...
function addProject(){
	 	...
	 	<form id="project-form">
		 	...
		 	<div class="row">
		 		<div class="twelve columns">
					...
					<textarea id="project-staff" type="text" class="form-control" placeholder="Project staff emails separated by semicolon: i.e. abc@abc.com; bbc@bbc.com"></textarea>
		 		</div>
		 	</div>		
		</form>
	...
	addStageButton.on('click', function(event) {
		...
		addProjectDetails.append(
			`
			<form id="stage-form">
				<div class="addStageInputFields-substage">
					<label>Stage <b>${stageNumber}</b> Subject</label>
					<input class="substage-topic" placeholder="i.e. Building the foundation for lasting client relationship." type="text"></input>
				</div>
				<div id="addTargetInputFields-${stageNumber}" class="addTargetInputFields">
					<label>Goals to Attain</label>
					<div class="substageGoalFirstInput" id="substageGoalFirstInput-${stageNumber}-${goalNumber}">
						<input class="substage-goal" placeholder="i.e. Assistant PM will finish market research" type="text"></input>
						<button class="allocateGoalButton button-primary" onClick=allocateGoal(${stageNumber}, ${goalNumber})>Allocate</button>
					</div>
					<div class="substageGoalSecondInput" id="substageGoalSecondInput-${stageNumber}-${goalNumber}">
					</div>
				</div>
				<div class="addTarget-button-group">
					<button id="addTargetButton" class="button-primary" onClick="addTarget(${stageNumber}, ${goalNumber})"> Add Goals</button>
				</div>
			</form>
			`
		);
...
function allocateGoal(stageNumber, goalNumber) {
	event.preventDefault();
	// var stageNumberToUniqueQueryFirst = "substageGoalFirstInput-"+stageNumber;
	// var addTargetDestinationFirst = $(`#${stageNumberToUniqueQueryFirst}`);
	...
	<div class="funkyradio">
        <div class="funkyradio-default checkbox-element">
            <input type="checkbox" name="checkbox" id="checkbox1" unchecked/>
            <label for="checkbox1">Heesung Son</label>
        </div>
        <div class="funkyradio-primary checkbox-element">
            <input type="checkbox" name="checkbox" id="checkbox2" unchecked/>
            <label for="checkbox2">Kong Wan</label>
        </div>
        <div class="funkyradio-success checkbox-element">
            <input type="checkbox" name="checkbox" id="checkbox3" unchecked/>
            <label for="checkbox3">Jie (Jeremy) Huang</label>
        </div>
        <div class="funkyradio-danger checkbox-element">
            <input type="checkbox" name="checkbox" id="checkbox4" unchecked/>
            <label for="checkbox4">Ankit Bhargava</label>
        </div>
        <div class="funkyradio-warning checkbox-element">
            <input type="checkbox" name="checkbox" id="checkbox5" unchecked/>
            <label for="checkbox5">Won Jun Hong</label>
        </div>
        <div class="funkyradio-info checkbox-element">
            <input type="checkbox" name="checkbox" id="checkbox6" unchecked/>
            <label for="checkbox6">Sandra Bullock</label>
        </div>
    </div>
    `
	)
...
function addTarget(stageNumber) {
	...
	addTargetDestination.append(
		...
		<div class="row">
			<input class="substage-goal" placeholder="i.e. Assistant PM will finish market research" type="text"></input>
			<button class="allocateGoalButton button-primary" onClick=allocateGoal(${stageNumber})>Allocate</button>
			<div class="goalAllocater" id="goalAllocater-${stageNumber}"></div>
		</div>
		`
	);
}
			...
			projectPreview = $(
				`<ul class="project-preview">
					...
					<div class="project-preview-project-creator">
						...
						${projectList[projectKey].permission.length+1} Members
					</div>
				    <div class="progress">
...
function editProfileInfo() {
	...
	// updateUserDBInfo.update({ userName: "Richard Hong"})
	// userProfilePreview.empty();
}
...
function register_popup(id, name) {   
    ...
    <div class="popup-messages">
    	...
    	<textarea class=chatInputBox id=${idSpecialCharacterRemoved}chatInputBox placeholder="Press enter to message ${name}.\n\nDouble click bubbles to edit sent messages." textarea rows="1" onkeydown="if (event.keyCode == 13) { submitChat('${id}', '${idSpecialCharacterRemoved}') }"></textarea>
    </div>
    </div>
    `
